...
namespace Kociemba
{
    public class SearchRunTime
    {
            ...
            */
            ...
            for (int i = 0; i < length; ++i) {
                ...
                switch (po[i]) {
                    case 1:     // clockwise
                        break;
                    case 2:     // Double movement (e.g. U2 -> uu)
                        sList.Add(sList[^1]);
                        break;
                    case 3:     // Counter-clockwise
                        sList[^1] = char.ToUpper(sList[^1]);
                        break;
                }
            }
        ...
        /// </summary>
        /// <param name="facelets">The cube definition string (see <see cref="Facelet"/> for the format).</param>
        /// <param name="maxDepth">The maximal allowed maneuver length.</param>
        /// <param name="timeOutMillis">The maximum computing time of the method in seconds.</param>
        /// <param name="useSeparator">Determines if a " . " separates the phase1 and phase2 parts of the solver string.</param>
        /// <returns>
        /// The solution string or an error code:
        /// <br>Error 1: There is not exactly one facelet of each color.</br>
        /// <br>Error 2: Not all 12 edges exist exactly once.</br>
        /// <br>Error 3: Flip error: One edge has to be flipped.</br>
        /// <br>Error 4: Not all corners exist exactly once.</br>
        /// <br>Error 5: Twist error: One corner has to be twisted.</br>
        /// <br>Error 6: Parity error: Two corners or two edges have to be exchanged.</br>
        /// <br>Error 7: No solution exists for the given maxDepth.</br>
        /// <br>Error 8: Timeout, no solution within the given time.</br>
        /// </returns>
        public static string solution(string facelets, out string info, int maxDepth= 22, long timeOutMillis = 10000, bool useSeparator = false, bool buildTables = false )
        {
            ...
            {
                info += "Error 1: There is not exactly one facelet of each colour.";
                return "";
            }
            for (int i = 0; i < 6; i++)
            {
            ...
            if ((s = cc.verify()) != 0)
            {
                ...
                {
                    case 2:
                        info = "Error 2: Not all 12 edges exist exactly once.";
                        break;
                    case 3:
                        info = "Error 3: Flip error: One edge has to be flipped.";
                        break;
                    case 4:
                        info = "Error 4: Not all corners exist exactly once.";
                        break;
                    case 5:
                        info = "Error 5: Twist error: One corner has to be twisted.";
                        break;
                    case 6:
                        info = "Error 6: Parity error: Two corners or two edges have to be exchanged.";
                        break;
                    default:
                        info = "Error: Unknown error. Probably a programming fault.";
                        break;
                }
                return "";
            }
            ...
            do
            {
                    ...
                    else if (++po[n] > 3)
                    {
                        ...
                        { // increment axis
                            if (++ax[n] > 5)
                            {
                                ...
                                if (DateTimeHelper.CurrentUnixTimeMillis() - tStart > timeOutMillis)
                                {
                                    info = "Error 8: Timeout, no solution within given time.";
                                    return "";
                                }
                                ...
                                if (n == 0)
                                {
                                    ...
                                    {
                                        info = "Error 7: No solution exists for the given maxDepth.";
                                        return "";
                                    }
                                    else
                                    {
                ...
                if (minDistPhase1[n + 1] == 0 && n >= depthPhase1 - 5)
                {
                    ...
                    if (n == depthPhase1 - 1 && (s = totalDepth(depthPhase1, maxDepth)) >= 0)
                    {
                        ...
                        {
                            info += "[ Solving time: " + String.Format(@"{0:mm\:ss\.ffff}", (DateTime.Now - startTime)) + " ]\n";
